...
public class SteamGeneratorBlockEntity extends TickableInventoryBlockEntity<SteamGeneratorBlockEntity.SteamGeneratorInventory> implements ICalendarTickable
{
    ...
    public static void serverTick(Level level, BlockPos pos, BlockState state, SteamGeneratorBlockEntity generator) {
        ...
        if (generator.temperature > 0) {
            // generator turns on aka becomes lit
        ...
        if (generator.temperature > 100 && generator.ticks == 0 && hasEnoughFluid(generator)
            ...
            if(pressure < 0) pressure = 0;      //get rid of negative values

            if(generator.temperature >= 2014) {
            //at max temperature for coal, 1 FE every-other-tick
                generator.ENERGY_STORAGE.receiveEnergy(1,false);
                generator.ticks += pressure + 1;
            } else {
            //otherwise get 1 FE every-3rd-tick
                generator.ENERGY_STORAGE.receiveEnergy(1, false);
                generator.ticks += pressure + 2;
            }
    ...
    public void onCalendarUpdate(long ticks)
    {
        ...
        // generator has no fuel to consume, but it does drop the internal target and temperature over time.
    ...
    private final ModEnergyStorage ENERGY_STORAGE = new ModEnergyStorage(1000, 10) {
        @Override
